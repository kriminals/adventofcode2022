...
const findCommonItems = (rucksack1, rucksack2) => {
    const commonItems = new Set(rucksack1.split('').filter(item => rucksack2.includes(item)))
    return [...commonItems].join()
}

const priority = rucksack.reduce((acc, curr, i, sack) => 
    (i % 3 === 0) ? acc + getPriority(findCommonItems(findCommonItems(sack[i], sack[i + 1]), sack[i + 2])) : acc
        ,0)
