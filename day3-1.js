...
const priority = rucksack.reduce((acc, curr) => {
    const commonItem = curr.split('',curr.length / 2).find(item => curr.slice(curr.length / 2).includes(item))
    const charCode = commonItem.charCodeAt() 
    return (charCode > 90) ? acc + charCode - 96 : acc + charCode - 38 
}, 0)
