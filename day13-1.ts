const input = `[[[[8,6,7],9,7,10],[[2,2,4],0,[4,9,10],[4,8,1,1],9],5],[8,8,[5,7],1,3],[],[6,[1,[0,1],[6,10,9]]],[]]
...
console.log(packets);
const compare = (left: number | number[], right: number | number[]) => {
  const isLeftNumber: boolean = typeof left === "number";
  const isRightNumber: boolean = typeof right === "number";
  ...
  if (left instanceof Array) return compare(left.flat(), [right].flat());
  if (right instanceof Array) return compare([left].flat(), right.flat());
};
const result = packets.reduce(
  (acc, line, index) => acc + (compare(line.at(0), line.at(1)) ? index + 1 : 0),
  0
);
console.log(result);
export {};
