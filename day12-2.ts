...
// find all a's
const findAllA = (el: string, nodes: Map<Point, string>): Array<Point> => {
  let allA: Array<Point> = [];
  for (let [key, value] of Array.from(nodes.entries())) {
    if (value === el || value === "S") allA.push(key);
  }
  return allA;
};
const starts: Array<Point> = findAllA("a", nodes);
console.log(starts);
const end: Point = findElement("E", nodes);
const steps: number[] = starts
  .map((start) => shortestPath(start, end, adjacencyList))
  .sort((a, b) => b - a);
console.log(steps);
export {};
